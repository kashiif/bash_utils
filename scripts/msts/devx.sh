...
# Pre-req: jq-1.7.1
# TODO: change to use jq docker image
tfstatelock-qa-del() {
  if [ "$1" = "" ] ; then
...
tfstatelock-staging-del() {
  if [ "$1" = "" ] ; then
    echo "Usage: tfstatelock-staging-del <LockID-From-Terraform-Error-Message>";
    return 1;
  fi

  #  Working, has additional filter at end: '| .LockID.S'
  #  cat kk-tests.json | jq ".Items[] | select(.Info != null) | select(.Info.S != null) | select(.Info.S | contains(\"$1\")) | .LockID.S";
  RECORD_KEY=`aws dynamodb scan --table-name caasdevx-staging-terraform-state | jq ".Items[] | select(.Info != null) | select(.Info.S != null) | select(.Info.S | contains(\"$1\")) | .LockID.S"`;
  echo "RECORD_KEY: $RECORD_KEY"
  aws dynamodb delete-item --table-name caasdevx-staging-terraform-state --key "{ \"LockID\": {\"S\":$RECORD_KEY} }"
}
